...
import com.foodbox.foodcabby.adapter.OrderFlowAdapter;
import com.foodbox.foodcabby.build.api.ApiClient;
import com.foodbox.foodcabby.build.api.ApiInterface;
import com.foodbox.foodcabby.fragments.OrderViewFragment;
import com.foodbox.foodcabby.helper.CustomDialog;
import com.foodbox.foodcabby.helper.DataParser;
...
import com.foodbox.foodcabby.models.OrderFlow;
...
public class CurrentOrderDetailActivity extends AppCompatActivity implements OnMapReadyCallback, LocationListener,
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        orderFlowList.add(new OrderFlow(getString(R.string.order_processed), getString(R.string.description_3), R.drawable.ic_order_processed, GlobalData.ORDER_STATUS.get(2) + GlobalData.ORDER_STATUS.get(3) + GlobalData.ORDER_STATUS.get(4)));
        orderFlowList.add(new OrderFlow(getString(R.string.order_pickedup), getString(R.string.description_4), R.drawable.ic_order_picked_up, GlobalData.ORDER_STATUS.get(5) + GlobalData.ORDER_STATUS.get(6)));
    ...
    private void cancelOrder(String reason) {
        ...
        Call<Order> call = apiInterface.cancelOrder(GlobalData.isSelectedOrder.getId(), reason);
            ...
            if (result != null) {
                ...
                if (result.size() > 0) {
                    for (int i = 0; i < result.size(); i++) {
                        ...
                        LatLng location = new LatLng(GlobalData.isSelectedOrder.getAddress().getLatitude(), GlobalData.isSelectedOrder.getAddress().getLongitude());
                        MarkerOptions markerOptions = new MarkerOptions()
                                .position(location).title("Source").draggable(true)
                                .icon(BitmapDescriptorFactory.fromResource(R.drawable.ic_hoem_marker));
                        sourceMarker = mMap.addMarker(markerOptions);

                        destLatLng = new LatLng(GlobalData.isSelectedOrder.getShop().getLatitude(), GlobalData.isSelectedOrder.getShop().getLongitude());
